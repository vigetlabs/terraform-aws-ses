...
| <a name="input_allowed_sending_addresses"></a> [allowed\_sending\_addresses](#input\_allowed\_sending\_addresses) | A list of email addresses that are allowed to send email from the domain.<br/>    If this list is empty, the domain will be configured to allow any email address to send email from the domain. | `list(string)` | `[]` | no |
